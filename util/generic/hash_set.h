...
{
private:
  typedef yhashtable<Value, Value, HashFcn, TIdentity<Value>,
                    EqualKey, Alloc> ht;
  ht rep;
...
public:
  ...
  iterator find_t(const TheKey& key) const { return rep.find(key); }
  template<class TheKey>
  ...
  TPair<iterator, iterator> equal_range(const key_type& key) const
