...
public:
  ...
  TPair<iterator, bool> insert_unique(const value_type& obj)
  {
  ...
  TPair<iterator, bool> insert_unique_noresize(const value_type& obj);
  ...
  template <class TheValue>
  reference find_or_insert(const TheValue& v);
  ...
  template<class TheKey>
  iterator find_i(const TheKey& key, insert_ctx &ins);
  ...
  int save_for_st(TStlSaver &saver, KeySaver &ks) const;
...
private:
  ...
  template<class TheKey>
  size_type bkt_num_key(const TheKey& key, size_t n) const
  {
    return hash(key) % n;
  }
...
TPair<__yhashtable_iterator<V>, bool>
yhashtable<V, K, HF, Ex, Eq, A>::insert_unique_noresize(const value_type& obj)
{
...
template <class V, class K, class HF, class Ex, class Eq, class A>
__yhashtable_iterator<V>
yhashtable<V, K, HF, Ex, Eq, A>::insert_equal_noresize(const value_type& obj)
{
  const size_type n = bkt_num(obj);
  node* first = buckets[n];
...
yhashtable<V, K, HF, Ex, Eq, A>::find_or_insert(const TheValue& v)
{
  resize(num_elements + 1);
...
template<class TheKey>
__yhashtable_iterator<V>
yhashtable<V, K, HF, Ex, Eq, A>::find_i(const TheKey& key, /*typename yhashtable<V, K, HF, Ex, Eq, A>::*/insert_ctx &ins)
{
  resize(num_elements + 1);
...
template <class V, class K, class HF, class Ex, class Eq, class A>
TPair<__yhashtable_iterator<V>, __yhashtable_iterator<V> >
yhashtable<V, K, HF, Ex, Eq, A>::equal_range(const key_type& key)
{
...
template <class V, class K, class HF, class Ex, class Eq, class A>
TPair<__yhashtable_const_iterator<V>, __yhashtable_const_iterator<V> >
yhashtable<V, K, HF, Ex, Eq, A>::equal_range(const key_type& key) const
{
...
template <class V, class K, class HF, class Ex, class Eq, class A>
typename yhashtable<V, K, HF, Ex, Eq, A>::size_type
yhashtable<V, K, HF, Ex, Eq, A>::erase(const key_type& key)
{
  const size_type n = bkt_num_key(key);
  node* first = buckets[n];
  size_type erased = 0;
...
template <class V, class K, class HF, class Ex, class Eq, class A>
void yhashtable<V, K, HF, Ex, Eq, A>::resize(size_type num_elements_hint)
{
...
template <class V, class K, class HF, class Ex, class Eq, class A>
void yhashtable<V, K, HF, Ex, Eq, A>::clear()
{
  const size_type buckets_size_minus_1 = buckets.size() - 1;
  for (size_type i = 0; i < buckets_size_minus_1; ++i) { /*y*/
    node* cur = buckets[i];
    if (cur) /*y*/
...
{
private:
  typedef yhashtable<TPair<const Key, T>, Key, HashFcn,
                     TSelect1st<TPair<const Key, T> >, EqualKey, Alloc > ht;
  ht rep;
...
public:
  ...
  template<class TheKey>
  iterator find_t(const TheKey& key) { return rep.find(key); }
  const_iterator find(const key_type& key) const { return rep.find(key); }
  template<class TheKey>
  const_iterator find_t(const TheKey& key) const { return rep.find(key); }
  template<class TheKey>
  ...
  template<class TheKey>
  T& operator[](const TheKey& key)
  {
    return rep.find_or_insert(TPairOfRefs<TheKey, T>(key, T())).second;
  }
  ...
  template<class KeySaver>
  int save_for_st(TStlSaver &saver, KeySaver &ks) const { return rep.save_for_st<KeySaver>(saver, ks); }
