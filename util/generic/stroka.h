...
#   define COMMA                      ,
#   define CODEPAGE_ARGUMENT_DEFAULT  const CodePage&cp=csYandex
#   define CODEPAGE_ARGUMENT_VAR  cp
#   define STROKA_INLINE
#else
#   define COMMA
#   define CODEPAGE_ARGUMENT_DEFAULT
#   define CODEPAGE_ARGUMENT_VAR
#   define STROKA_INLINE                     inline
#endif
...
struct TStringData {
    typedef TStringData<TCharType> TSelf;

    TAtomic Refs;
    size_t BufLen;    //!< maximum number of characters that this data can fit
    size_t Length;          //!< actual string data length
    TCharType Chars[1];     //!< the first character of the string data

    static TCharType* GetNull() {
        static char null[sizeof(TSelf)];
        return ReinterpretCast<TSelf*>(null)->Chars;
    }
};

template <typename TCharType>
struct TStringDataTraits {
    typedef TStringData<TCharType> TData;

    enum {
        OffsetOfChars = offsetof(TData, Chars),
        Overhead = OffsetOfChars + sizeof(TCharType) // + null terminated symbol
    };

    static const size_t MaxSize = (size_t(-1) - Overhead) / sizeof(TCharType);

    static size_t CalcDataSize(size_t len) {
        return Max(FastClp22(len) * 2 + Overhead, len);
    }

    static TData* GetData(TCharType* p) {
        // 'p' must be casted to char* to calculate correct address of TStringData
        return reinterpret_cast<TData*>(reinterpret_cast<char*>(p) - OffsetOfChars);
    }
};
...
template <typename TCharType>
TCharType* Allocate(size_t oldLen, size_t newLen, TStringData<TCharType>* oldData = NULL);

void Deallocate(void* data);

} // namespace NDetail

template < typename TDerived, typename TCharType, typename TTraits > class TStringBase;
...
protected:
    ...
    //! @throw std::length_error
    static TCharType* Allocate(size_t len, TData* oldData = NULL) {
        return NDetail::Allocate(len, len, oldData);
    }
    ...
    inline void UnRef() throw () {
        if (p != TData::GetNull()) {
            if (AtomicDecrement(GetData()->Refs) == 0) {
...
public:
    ...
    explicit TStringImpl(const TFixedString& s1, const TFixedString& s2) {
        p = Allocate(s1.Length + s2.Length);
        TTraits::Copy(p, s1.Start, s1.Length);
        TTraits::Copy(p + s1.Length, s2.Start, s2.Length);
    }

    explicit TStringImpl(const TFixedString& s1, const TFixedString& s2, const TFixedString& s3) {
        p = Allocate(s1.Length + s2.Length + s3.Length);
        TTraits::Copy(p, s1.Start, s1.Length);
        TTraits::Copy(p + s1.Length, s2.Start, s2.Length);
        TTraits::Copy(p + s1.Length + s2.Length, s3.Start, s3.Length);
    }
    ...
    inline void clear() throw () {
        ...
        Relink(TData::GetNull());
    }

    template <class T1, class T2>
    static inline TDerived Join(const T1& t1, const T2& t2) {
        return TDerived(TFixedString(t1), TFixedString(t2));
    }

    template <class T1, class T2, class T3>
    static inline TDerived Join(const T1& t1, const T2& t2, const T3& t3) {
        return TDerived(TFixedString(t1), TFixedString(t2), TFixedString(t3));
    }
    ...
    TDerived & assign(const TFixedString & s
        , size_t spos, size_t sn = TBase::npos)
    {
    ...
    inline void reserve(size_t len) {
        if (IsNonShared()) {
            if (capacity() < len) {
                p = NDetail::Allocate(length(), len, GetData());
            }
        } else {
            Relink(TTraits::Copy(NDetail::Allocate(length(), len, (TData*)NULL), p, length()));
        }
    }
    ...
    inline void ReserveAndResize(size_t len) {
        if (IsNonShared()) {
            ...
            Relink(TTraits::Copy(Allocate(len), p, length()));
        }
    }
    ...
    TDerived & prepend(const TFixedString & s
        , size_t spos = 0, size_t sn = TBase::npos)
    {
    ...
    TDerived& remove(size_t pos, size_t n) {
        return replace(pos, n, TData::GetNull(), 0, 0, 0);
    }
    ...
    TDerived & replace(size_t pos, size_t n
        , const TCharType * s, size_t len)
    {
        return replace(pos, n, s, 0, len, len);
    }

    TDerived & replace(size_t pos, size_t n
        , const TCharType * s, size_t spos, size_t sn)
    {
    ...
    TDerived & replace(size_t pos, size_t n, const TFixedString & s
        , size_t spos = 0, size_t sn = TBase::npos)
    {
...
public:
     ...
     */
    bool to_lower(size_t pos = 0, size_t n = TBase::npos COMMA CODEPAGE_ARGUMENT_DEFAULT);
    bool to_upper(size_t pos = 0, size_t n = TBase::npos COMMA CODEPAGE_ARGUMENT_DEFAULT);
    bool to_title(CODEPAGE_ARGUMENT_DEFAULT);
    // @}
    ...
    friend Stroka to_lower(const Stroka& s COMMA CODEPAGE_ARGUMENT_DEFAULT) {
        Stroka ret(s);
        ret.to_lower(0, TBase::npos COMMA CODEPAGE_ARGUMENT_VAR);
        return ret;
    }

    friend Stroka to_upper(const Stroka& s COMMA CODEPAGE_ARGUMENT_DEFAULT) {
        Stroka ret(s);
        ret.to_upper(0, TBase::npos COMMA CODEPAGE_ARGUMENT_VAR);
        return ret;
    }

    friend Stroka to_title(const Stroka& s COMMA CODEPAGE_ARGUMENT_DEFAULT) {
        Stroka ret(s);
        ret.to_title(CODEPAGE_ARGUMENT_VAR);
        return ret;
    }
...
public:
    ...
    static STROKA_INLINE int compare(const TFixedString &p1, const TFixedString &p2 COMMA CODEPAGE_ARGUMENT_DEFAULT);

    static bool is_prefix(const TFixedString &what, const TFixedString &of COMMA CODEPAGE_ARGUMENT_DEFAULT);
    static bool is_suffix(const TFixedString &what, const TFixedString &of COMMA CODEPAGE_ARGUMENT_DEFAULT);

    bool is_prefix(const TFixedString &s COMMA CODEPAGE_ARGUMENT_DEFAULT) const {
        return is_prefix(*this, s COMMA CODEPAGE_ARGUMENT_VAR);
    }
    bool is_suffix(const TFixedString &s COMMA CODEPAGE_ARGUMENT_DEFAULT) const {
        return is_suffix(*this, s COMMA CODEPAGE_ARGUMENT_VAR);
    }

    bool has_prefix(const TFixedString &s COMMA CODEPAGE_ARGUMENT_DEFAULT) const {
        return is_prefix(s, *this COMMA CODEPAGE_ARGUMENT_VAR);
    }
    bool has_suffix(const TFixedString &s COMMA CODEPAGE_ARGUMENT_DEFAULT) const {
        return is_suffix(s, *this COMMA CODEPAGE_ARGUMENT_VAR);
    }
