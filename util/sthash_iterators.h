...
/**
    This file provides functionality for saving some relatively simple yhash object
    ...
    the same parameters as original yhash object)
    ...
    TSthashWriter (write iterator for yhash).
...
struct TSthashWriter {
    ...
    size_t GetRecordSize(const TPair<const Key, const Value> &) const {
        return sizeof(Key) + SizeOfEx<Value>();
    }
    template<class TSaver>
    int SaveRecord(TSaver &saver, const TPair<const Key, const Value> &record) const {
        saver.Save(&record.first, sizeof(Key));
        saver.Save(&record.second, SizeOfEx<Value>());
        return 0;
    }
};


// Remember that this simplified implementation makes a copy of `key' in MakePair.
...
struct TSthashSetWriter : public TSthashWriter<Key, TEmptyValue, size_type_o> {
    ...
    size_t GetRecordSize(const Key &key) const {
        return MapWriter::GetRecordSize(MakePair(key, TEmptyValue()));
    }
    template<class TSaver>
    int SaveRecord(TSaver &saver, const Key &key) const {
        return MapWriter::SaveRecord(saver, MakePair(key, TEmptyValue()));
    }
};
...
struct TSthashWriter<const char *, Value, size_type_o> {
    typedef size_type_o TSizeType;
    size_t GetRecordSize(const TPair<const char *, const Value> &record) const {
        size_t length = strlen(record.first) + 1 + SizeOfEx<Value>();
        length = AlignUp(length, AlignForChrKey<Value>());
        return length;
    }
    template<class TSaver>
    int SaveRecord(TSaver &saver, const TPair<const char *, const Value> &record) const {
        const char *alignBuffer = "qqqq";
        saver.Save(&record.second, SizeOfEx<Value>());
        size_t length = strlen(record.first) + 1;
        saver.Save(record.first, length);
        length = AlignUpSpace(length, AlignForChrKey<Value>());
        if (length)
            saver.Save(alignBuffer, length);
        return 0;
    }
};
...
struct TSthashWriter<Key, const char*, size_type_o> {
    typedef size_type_o TSizeType;
    size_t GetRecordSize(const TPair<const Key, const char *> &record) const {
        size_t length = strlen(record.second) + 1 + sizeof(Key);
        length = AlignUp(length, (size_t)4);
        return length;
    }
    template<class TSaver>
    int SaveRecord(TSaver &saver, const TPair<const Key, const char *> &record) const {
        const char *alignBuffer = "qqqq";
        saver.Save(&record.first, sizeof(Key));
        size_t length = strlen(record.second) + 1;
        saver.Save(record.second, length);
...
struct TSthashWriter<const char*, const char*, size_type_o> {
    typedef size_type_o TSizeType;
    size_t GetRecordSize(const TPair<const char *, const char *> &record) const {
        size_t size = strlen(record.first) + strlen(record.second) + 2;
        return size;
    }
    template<class TSaver>
    int SaveRecord(TSaver &saver, const TPair<const char *, const char *> &record) const {
        saver.Save(record.first, strlen(record.first) + 1);
        saver.Save(record.second, strlen(record.second) + 1);
        return 0;
    }
};
