...
public:
    inline TImpl(TOutputStream* out, ZLib::StreamType type, size_t compression_level)
        : Stream_(out)
    {
        if (type == ZLib::Auto) {
            type = ZLib::ZLib;
        ...
        compression_level = Min<size_t>(9, compression_level);

        if (deflateInit2(Z(), compression_level, Z_DEFLATED, opts[type], 9, Z_DEFAULT_STRATEGY)) {
...
TZLibCompress::TZLibCompress(TOutputStream* out, ZLib::StreamType type, size_t compression_level, size_t buflen)
    : Impl_(new (buflen) TImpl(out, type, compression_level))
{
}
