...
TStr JoinStrokuT(const yvector<TStr>& v, size_t index, size_t count, const TStr& delim) {
    TStr result;
    for (size_t i = 0; i < count && index < v.size(); ++i, ++index) {
        if (i > 0)
            result.append(delim);
