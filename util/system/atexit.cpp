...
class TRecursiveLock {
        typedef typename TThread::TId TThrId;
    public:
        inline TRecursiveLock()
            : ThreadId_(TThread::ImpossibleThreadId())
            , RecurseLevel_(0)
        {
        }

        inline void Acquire() throw () {
            const TThrId cur = TThread::CurrentThreadId();
        ...
        inline void Release() throw () {
            YASSERT(ThreadId_ == TThread::CurrentThreadId());
            YASSERT(RecurseLevel_ > 0);
    ...
    private:
        TLock Lock_;
        volatile TThrId ThreadId_;
        size_t RecurseLevel_;
};

class TAtExit {
        typedef TRecursiveLock<TAdaptiveLock> TLock;
    public:
