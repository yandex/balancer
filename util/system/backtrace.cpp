...
#if !defined(HAVE_BACKTRACE) && (defined(_glibc_) || defined(_darwin_))
...
size_t BackTrace(void** p, size_t len) {
    return Singleton<TBackTrace>()->BackTrace(p, len);
}
#endif
...
void FormatBackTrace(TOutputStream* out) {
    ...
    for (size_t i = 0; i < s; ++i) {
        TResolvedSymbol rs = ResolveSymbol(array[i], tmpBuf, sizeof(tmpBuf));

        *out << rs.Name << "+" << ((long)array[i] - (long)rs.NearestSymbol) << Endl;
    }
}
