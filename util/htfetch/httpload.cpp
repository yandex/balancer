...
long httpAgentReader::readPortion(void *&buf)
{
    ...
    do
    {
        ...
        if (State == hp_error &&
           Header_.error==HTTP_PAST_EOF &&
           Header_.entity_size > Header_.content_length)
        {
...
/************************************************************/
bool httpLoadAgent::startRequest
    (const THttpURL& url,
     bool            persistent,
     ui32            ip)
{
    ...
    return doSetHost(ip) && doStartRequest();
}
...
/************************************************************/
bool httpLoadAgent::doSetHost(ui32 ip)
{
    ...
    if (!URL_.IsNull(THttpURL::FlagAuth))
    {
        ...
        Digest_.setAuthorization
            (URL_.Get(THttpURL::FieldUsername),
...
bool httpLoadAgent::startOneRequest(const char* local_url)
{
    ...
    THttpURL lURL;
    if (lURL.Parse(local_url, THttpURL::FeaturesNormalizeSet) ||
        lURL.IsValidGlobal())
        return false;
...
{
    Stroka urlStr = URL_.PrintS(THttpURL::FlagPath|THttpURL::FlagQuery);
    if (!urlStr)
        urlStr = "/";
    ...
    for (int step = 0; step<10; step++)
    {
        ...
        ErrCode_ = RequestGet
            (urlStr.c_str(), headers, PersistentConn_);
        ...
        Stroka urlBaseStr = URL_.PrintS(THttpURL::FlagNoFrag);

        clearReader();
        Reader_ = new httpAgentReader
            (*this, urlBaseStr.c_str(),
