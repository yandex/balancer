...
struct TFakeCheck {
    bool   Check(THttpHeader * /*header*/) { return false;}
    void   CheckDocPart(void * /*buf*/, size_t /*len*/, THttpHeader * /*header*/) {} //for every part of DocumentBody will be called
    void   CheckEndDoc(THttpHeader * /*header*/) { }
    size_t GetMaxHeaderSize()             { return headermax; }
    size_t GetMaxBodySize(THttpHeader*)   { return bodymax; }
};
...
public:
    ...
    THttpHeader *GetHttpHeader() const { return Header; }

protected:

    int CheckHeaders() {
        if (
            Header->http_status < 200 || Header->http_status == 204
            || Header->http_status == 304
        ) {
        ...
        if (
           !Header->transfer_chunked &&
           !Header->connection_closed && Header->content_length < 0 &&
           !HeadRequest
        ) {
        ...
        while (size) {
            ...
            switch (State) {
                ...
                case hp_read_alive:
                    ...
                    if (Header->entity_size > Header->content_length) { //error possible wrong logic (on return and CheckDocPart values)
                        Header->error = HTTP_PAST_EOF;
                        State = hp_error;
                    } else if (Header->entity_size == Header->content_length) {
                        State = hp_eof;
                    }
...
public:
    ...
    int Init(THttpHeader *H, int parsHeader) {
        Header = H;
        Eoferr = 1;
        Size = 0;
        AssumeConnectionClosed = 0;
        HeadRequest = false;
