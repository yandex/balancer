...
public:
    ...
    int Init(THttpHeader *H, int parsHeader, const size_t maxContSize = (size_t)-1,
            const size_t bufSize = 64<<10, const unsigned int winSize = 15) {
        ZErr = 0;
        CurContSize = 0;
        MaxContSize = maxContSize;

        int ret = TBase::Init(H, parsHeader);
        if (ret)
            return ret;
    ...
    long Read(void *&buf) {
		...
		Stream.next_in = (arc_Bytef*)tmpin;
                if (res <= 0)
                    return res;
            ...
            switch (err) {
                case Z_OK:
                    ...
                    if (BufSize == Stream.avail_out)
                        continue;
                    // don't break or return; continue with Z_STREAM_END case
...
public:
    ...
    int Init(THttpHeader *H, int parsHeader, const size_t maxContSize = (size_t)-1,
            const size_t bufSize = 64<<10, const unsigned int winSize = 15) {
        int ret = TBase::Init(H, parsHeader, maxContSize, bufSize, winSize);
