...
#include <util/htfetch/httpheader.h>
#include <util/htfetch/httpfsm.h>
...
include HttpDateTimeParser "../datetime/parser.rl";
...
action set_request_uri {
    if (request_hd && buflen < URL_MAX) {
        buf[buflen++] = 0;
        if (request_hd->request_uri[0] != 0) {
            return -2;
        }
        memcpy(request_hd->request_uri, buf, buflen);
    }
}
...
################# location ########################
action set_location {
    if (hd && buflen < URL_MAX) {
        buf[buflen++] = 0;
        memcpy(hd->location, buf, buflen);
    }
}
...
location        = "location"i def url eoh %set_location;
refresh         = "refresh"i def int ';' lws "url="i url eoh %set_location %set_status_303;
