...
/********************************************************/
int socketSecureHandler::Connect
    (ui32 Ip, TIpPort Port, time_t Timeout)
{
    IsValid_ = false;

    int ret = socketRegularHandler::Connect(Ip, Port, Timeout);
    if (ret)
        return ret;
...
/************************************************************/
socketAbstractHandler* socketHandlerFactory::chooseHandler
   (const THttpURL& url)
{
    if (url.IsValidGlobal() &&
       url.GetScheme() == THttpURL::SchemeHTTP)
        return new socketRegularHandler;

    #ifdef USE_GNUTLS
    if (url.IsValidGlobal() &&
       url.GetScheme() == THttpURL::SchemeHTTPS)
