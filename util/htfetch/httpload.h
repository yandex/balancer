...
public:
    ...
    virtual int Connect(ui32 Ip, TIpPort Port, time_t Timeout) = 0;
...
public:
    ...
    {
        return (Handler_)? Handler_->Connect(Ip, Port, Timeout):1;
    }
...
// ant its derivatives
typedef THttpAgent<TSocketHandlerPtr> httpSpecialAgent;
...
class socketRegularHandler: public socketAbstractHandler
{
  ...
  public:
    ...
    virtual int Connect(ui32 Ip, TIpPort Port, time_t Timeout)
    {
        return Socket_.Connect(Ip, Port, Timeout);
    }
...
class socketHandlerFactory
{
  public:
    ...
    virtual socketAbstractHandler* chooseHandler
        (const THttpURL& url);
...
class httpLoadAgent: public httpSpecialAgent
{
  protected:
    ...
    VectorStrok           Headers_;
    int                   ErrCode_;
    char*                 RealHost_;
    httpDigestHandler     Digest_;

    void clearReader();
    bool doSetHost(ui32 ip);
    bool doStartRequest();

  public:
    httpLoadAgent
        (bool                   handleAuthorization = false,
