...
protected:
    void check_capacity(size_t len) {
        if (EXPECT_FALSE(!last_free || len > last_free)) {
            ...
            assert(curseg->freepos == 0);
            assert(curseg->_size >= last_free);
        }
    }
public:
    explicit
    ...
    void undo_last_append() {
        assert(curseg != segs.end()); // do not use before append()
        if (last_ins_size) {
            assert(last_ins_size <= curseg->freepos);
    ...
    void alloc_first_seg() {
        ...
        assert(capacity() == segment_size * sizeof(T));
    }
