...
class TAllocedBuf: public TTempBuf::TImpl,
                   public TAdditionalStorage<TAllocedBuf> {
    public:
        inline TAllocedBuf()
...
class TPerThreadedBuf: public TTempBuf::TImpl,
                       public TIntrusiveSListItem<TPerThreadedBuf> {
...
#if defined(_win_) || defined(_sun_) || defined(__sun__)
static inline TTempBufManager*& MyManager(){
    STATIC_THREAD(TTempBufManager*) my_manager((TTempBufManager*)NULL);

    return my_manager;
}
#define my_manager MyManager()
#else
POD_STATIC_THREAD(TTempBufManager*) my_manager((TTempBufManager*)NULL);
#endif

static inline TTempBufManager& MyManagerImpl() {
    STATIC_THREAD(TTempBufManager) my_manager_impl;

    return my_manager_impl;
}
#define my_manager_impl MyManagerImpl()

static inline TTempBufManager* TempBufManager() {
    if (!my_manager) {
        my_manager = &(TTempBufManager&)my_manager_impl;
    }
