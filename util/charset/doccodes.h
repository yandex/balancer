...
enum ECharset{
    ...
    CODES_MAC_THAI,
    CODES_MAC_TURKISH,
    CODES_RESERVED_2, // reserved code: use it for new encodings before adding them to the end of the list
    CODES_MULELAO,
    CODES_NEXTSTEP,
    CODES_PT154,
    CODES_RISCOS_LATIN1,
...
inline MimeTypes mimeByStr(const char *mimeStr) {
    ...
    if (strcmp(mimeStr, "application/x-gzip") == 0)
        return MIME_GZIP;
    if (strcmp(mimeStr, "text/javascript") == 0 || strcmp(mimeStr, "application/javascript") == 0)
        return MIME_JAVASCRIPT;
    if (strcmp(mimeStr, "application/epub+zip") == 0)
        return MIME_EPUB;
    if (strcmp(mimeStr, "application/tex") == 0)
        return MIME_TEX;
    return MIME_UNKNOWN;
}

inline const char* strByMime(MimeTypes mime) {
    const char *res = 0;
    if (mime == MIME_HTML)
        res = "text/html";
        ...
        res = "application/epub+zip";
    else if (mime == MIME_TEX)
        res = "application/tex";
    return res;
}
