...
inline size_t WideToChar(const wchar16* text, size_t len, char* dest, ECharset enc = CODES_YANDEX) {
    YASSERT(SingleByteCodepage(enc));

    const char* start = dest;

    const Encoder* const encoder = &EncoderByCharset(enc);
    const wchar16* const last = text + len;
    for (const wchar16* cur = text; cur != last; ++dest) {
...
inline void CharToWide(const char* text, size_t len, wchar16* dest, const CodePage& cp = csYandex) {
    ...
    for (; cur != last; ++cur, ++dest) {
        *dest = static_cast<wchar16>(cp.unicode[*cur]); // static_cast is safe as no 1char codepage contains non-BMP symbols
    }
}
...
inline void WideToUTF8(const wchar16* text, size_t len, char* dest, size_t& written) {
    const wchar16* const last = text + len;
    unsigned char* p = reinterpret_cast<unsigned char*>(dest);
    size_t runeLen;
    for (const wchar16* cur = text; cur != last;) {
...
inline size_t UTF8ToWideImpl(const char* text, size_t len, wchar16* dest, size_t& written) {
    const unsigned char* cur = reinterpret_cast<const unsigned char*>(text);
    const unsigned char* const last = cur + len;
    wchar16* p = dest;
...
template <bool robust>
inline bool UTF8ToWide(const char* text, size_t len, wchar16* dest, size_t& written) {
...
namespace NDetail {
    ...
    namespace NBaseOps {
        ...
        inline TStringBufImpl<TCharTo> RecodeUtf8(const TStringBufImpl<TCharFrom>& src, TCharTo* dst);

        template <typename TCharFrom, typename TCharTo>
        inline TStringBufImpl<TCharTo> RecodeSingleByteChar(const TStringBufImpl<TCharFrom>& src, TCharTo* dst, const CodePage& cp);

        template <typename TCharFrom, typename TCharTo>
        inline TStringBufImpl<TCharTo> RecodeMultiByteChar(const TStringBufImpl<TCharFrom>& src, TCharTo* dst, ECharset encoding);
        ...
        TStringBufImpl<TCharTo> Recode(const TStringBufImpl<TCharFrom>& src, TCharTo* dst, ECharset encoding) {
            if (encoding == CODES_UTF8)
                return RecodeUtf8<TCharFrom, TCharTo>(src, dst);
            else if (SingleByteCodepage(encoding))
                return RecodeSingleByteChar<TCharFrom, TCharTo>(src, dst, *CodePageByCharset(encoding));
            else
                return RecodeMultiByteChar<TCharFrom, TCharTo>(src, dst, encoding);
        }


        template <>
        inline TWtringBuf RecodeSingleByteChar(const TStringBuf& src, wchar16* dst, const CodePage& cp) {
            YASSERT(cp.SingleByteCodepage());
            ::CharToWide(~src, +src, dst, cp);
            return TWtringBuf(dst, +src);
        }

        template <>
        inline TStringBuf RecodeSingleByteChar(const TWtringBuf& src, char* dst, const CodePage& cp) {
            YASSERT(cp.SingleByteCodepage());
        ...
        template <>
        inline TWtringBuf RecodeMultiByteChar(const TStringBuf& src, wchar16* dst, ECharset encoding) {
            YASSERT(!NCodepagePrivate::NativeCodepage(encoding));
            size_t read = 0;
            size_t written = 0;
            ::NICONVPrivate::RecodeToUnicode(encoding, ~src, dst, +src, +src, read, written);
            return TWtringBuf(dst, written);
        }

        template <>
        inline TStringBuf RecodeMultiByteChar(const TWtringBuf& src, char* dst, ECharset encoding) {
            YASSERT(!NCodepagePrivate::NativeCodepage(encoding));
            size_t read = 0;
            size_t written = 0;
        ...
        template <>
        inline TWtringBuf RecodeUtf8(const TStringBuf& src, wchar16* dst) {
            size_t len = 0;
            if (!::UTF8ToWide(~src, +src, dst, len))
                ythrow yexception() << "Invalid UTF8: \"" << src.SubStr(0, 50) << (src.size() > 50 ? "...\"" : "\"");
            return TWtringBuf(dst, len);
        }

        template <>
        inline TStringBuf RecodeUtf8(const TWtringBuf& src, char* dst) {
            size_t len = 0;
