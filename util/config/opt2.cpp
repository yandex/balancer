...
const char *Opt2::Arg(char opt, const char *help, const char *def, bool required) {
    ...
    return p.IsFound? p.ActualValue : def;
}
...
const char *Opt2::Arg(char opt, const char *help, Stroka def, bool required) {
    ...
    return p.IsFound? p.ActualValue : p.DefValue;
}
...
long Opt2::Int(char opt, const char *help, long def, bool required) {
    ...
    p.IsNumeric = true;
    if (!p.IsFound || !p.ActualValue)
        return def;
    char *e;
    long rv = strtol(p.ActualValue, &e, 10);
    if (e == p.ActualValue || *e) {
        OptionWrongArg = opt;
        HasErrors = true;
    }
    return rv;
...
unsigned long Opt2::UInt(char opt, const char *help, unsigned long def, bool required) {
    ...
    if (!p.IsFound || !p.ActualValue)
        return def;
    char *e;
    unsigned long rv = strtoul(p.ActualValue, &e, 10);
    if (e == p.ActualValue || *e) {
        OptionWrongArg = opt;
        HasErrors = true;
    }
