...
public:
    ...
    template <class A>
    explicit
    autoarray(size_t siz, const A &fill)
    {
        AllocBuf(siz);
        T *curr = arr;
        try {
    ...
    explicit
    autoarray(size_t siz, const A &fill, const B &fill1)
    {
        AllocBuf(siz);
        T *curr = arr;
        try {
            for (T *end = arr + _size; curr != end; ++curr)
                new(curr) T(fill, fill1);
        } catch(...) {
            for (--curr; curr >= arr; --curr)
                curr->~T();
            y_deallocate(arr);
    ...
    explicit
    autoarray(size_t siz, const A &fill, const B &fill1, const C &fill2)
    {
        AllocBuf(siz);
        T *curr = arr;
        try {
            for (T *end = arr + _size; curr != end; ++curr)
                new(curr) T(fill, fill1, fill2);
        } catch(...) {
            for (--curr; curr >= arr; --curr)
                curr->~T();
            y_deallocate(arr);
    ...
    explicit
    autoarray(size_t siz, const A &fill, const B &fill1, const C &fill2, const D &fill3)
    {
        AllocBuf(siz);
        T *curr = arr;
        try {
            for (T *end = arr + _size; curr != end; ++curr)
                new(curr) T(fill, fill1, fill2, fill3);
        } catch(...) {
            for (--curr; curr >= arr; --curr)
                curr->~T();
            y_deallocate(arr);
