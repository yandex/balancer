/* $Id: output.c,v 1.3 2007-12-28 13:36:36 pg Exp $ */
...
static void output_rule_data(void)
{
    ...
    fprintf(output_file, "static const short %slhs[] = {%42d,", symbol_prefix,
    ...
    fprintf(output_file, "static const short %slen[] = {%42d,", symbol_prefix, 2);
...
static void output_yydefred(void)
{
    ...
    fprintf(output_file, "static const short %sdefred[] = {%39d,", symbol_prefix,
...
static void goto_actions(void)
{
    ...
    fprintf(output_file, "static const short %sdgoto[] = {%40d,", symbol_prefix, k);
...
static void output_base(void)
{
    ...
    fprintf(output_file, "static const short %ssindex[] = {%39d,", symbol_prefix, base[0]);
    ...
    if (!rflag)
	outline += 2;
    fprintf(output_file, "\n};\nstatic const short %srindex[] = {%39d,", symbol_prefix,
	    base[nstates]);
    ...
    if (!rflag)
	outline += 2;
    fprintf(output_file, "\n};\nstatic const short %sgindex[] = {%39d,", symbol_prefix,
	    base[2 * nstates]);
...
static void output_table(void)
{
    ...
    fprintf(output_file, "static const short %stable[] = {%40d,", symbol_prefix,
	    table[0]);
...
static void output_check(void)
{
    ...
    fprintf(output_file, "static const short %scheck[] = {%40d,", symbol_prefix,
	    check[0]);
...
void write_pure() {
    if (pure_parser) {
        fprintf(defines_file, "#undef YYPURE_PARSER\n");
        fprintf(defines_file, "#define YYPURE_PARSER 1\n");
        fprintf(code_file, "#undef YYPURE_PARSER\n");
        fprintf(code_file, "#define YYPURE_PARSER 1\n");
    }
}
...
static void output_defines(void)
{
    ...
    if (dflag && unionized)
    {
        fprintf(defines_file, "#if !defined(YYSTYPE_DEFINED)\n");
        rewind(union_file);
        while ((c = getc(union_file)) != EOF)
            putc(c, defines_file);
        fprintf(defines_file, " YYSTYPE;\nextern YYSTYPE %slval;\n",
            symbol_prefix);
        fprintf(defines_file, "#define YYSTYPE_DEFINED\n");
...
static void output_debug(void)
{
    ...
    if (!rflag)
	++outline;
    fprintf(output_file, "#if YYDEBUG\nstatic const char *%sname[] = {", symbol_prefix);
    j = 80;
    for (i = 0; i <= max; ++i)
    {
	if ((s = symnam[i]) != 0)
...
static void output_stype(void)
{
	...
	outline += 3;
    fprintf(code_file, "#ifndef YYSTACK\n#define YYSTACK ParserStack");

    for (i = 0; i < sizeof(buf) - 1; ++i) {
        buf[i] = '0' + (rand() % 10);
    }

    fprintf(code_file, buf);
