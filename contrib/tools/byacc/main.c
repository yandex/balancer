/* $Id: main.c,v 1.5 2007-12-28 13:36:36 pg Exp $ */
...
static const char *is_suffix(const char *s, const char *suff) {
    unsigned len = strlen(s);
    unsigned slen = strlen(suff);
    if (len >= slen && strcmp(s + len - slen, suff) == 0)
        return s + len - slen;
    return NULL;
}
...
static void open_files(void)
{
    ...
    action_file = fopen(action_file_name, "wb+");
    if (action_file == 0)
	...
	union_file = fopen(union_file_name, "wb+");
	if (union_file == 0)
